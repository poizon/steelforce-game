...
export interface GameEventData {
  ...
  [GameEvent.SCENE_TRANSITION_START]: { from?: string; to: string; type: string };
  ...
  [GameEvent.ASSETS_LOADING_COMPLETE]: { totalLoaded: number; bundles: string[] };
  ...
  [GameEvent.ASSETS_ASSET_ERROR]: { bundleName: string; assetAlias: string; error: Error };

  [GameEvent.AUDIO_PLAY_STARTED]: { alias: string; id: string; category: string };
  ...
  [GameEvent.PLAYER_MOVE]: { position: { x: number; y: number }; direction: string };
  [GameEvent.PLAYER_DAMAGE]: { amount: number; currentHealth: number; source?: string };
  [GameEvent.PLAYER_HEAL]: { amount: number; currentHealth: number };
  [GameEvent.PLAYER_DEATH]: { cause: string; position: { x: number; y: number } };
  ...
  [GameEvent.MONSTER_SPAWN]: { type: string; id: string; position: { x: number; y: number } };
  [GameEvent.MONSTER_DETECT]: { id: string; targetId: string };
  [GameEvent.MONSTER_ATTACK]: { id: string; targetId: string; damage: number };
  [GameEvent.MONSTER_DAMAGE]: { id: string; amount: number; currentHealth: number };
  [GameEvent.MONSTER_DEATH]: { id: string; type: string; position: { x: number; y: number } };
  ...
  [GameEvent.DIALOG_CHOICE]: { dialogueId: string; choiceId: string; choiceIndex: number };
  ...
  [GameEvent.UI_NOTIFICATION]: { message: string; type: 'info' | 'warning' | 'error' | 'success' };
...
type EventCallback<K extends GameEvent = GameEvent> =
  (data: GameEventData[K]) => void | Promise<void>;
...
export class EventBus {
  ...
  public off<K extends GameEvent>(
    ...
    if (typeof callbackOrId === 'string') {
      ...
      if (index !== -1) {
        ...
        if (this.debugMode) {
          console.log(`[EventBus] Unsubscribed from "${event}" (id: ${callbackOrId})`);
        }
      }
    } else {
      // Отписка по callback
      ...
      if (index !== -1) {
        ...
        listeners.splice(index, 1);
        if (this.debugMode) {
          console.log(`[EventBus] Unsubscribed from "${event}" (id: ${subscription.id})`);
        }
      }
    }
  }
  ...
  public offContext(context: string): void {
    ...
    if (this.debugMode) {
      console.log(`[EventBus] Removed ${removedCount} listeners for context "${context}"`);
    }
  }
